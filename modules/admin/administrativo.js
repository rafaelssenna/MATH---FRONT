...
// ╔═══════════════════════════════════════════════════════════════════════════════╗
// ║                    SEÇÕES 2-4: MOVIDAS PARA MÓDULOS                           ║
// ║    - js/utils.js: debounce, escapeHtml, formatHours, formatQuantity, etc.    ║
// ║    - js/icons.js: SVGIcons                                                    ║
// ║    - js/sidebar.js: toggleSidebar, closeSidebar                               ║
// ╚═══════════════════════════════════════════════════════════════════════════════╝

// NOTA: Funções utilitárias movidas para js/utils.js:
// - getBrazilianHolidays, isBusinessDay, getNextBusinessDay
// - calculateDueDates, formatDueDatesForPDF
// - debounce, escapeHtml, formatHours, formatQuantity
// - fetchWithTimeout, safeFetch
...
/**
 * Abre a página da cobrança na Conta Azul
 * Busca a URL do boleto via API e abre em nova aba
 */
async function abrirCobrancaContaAzul(cobrancaId) {
  try {
    // Mostra loading
    const btn = event?.target?.closest('button')
    if (btn) {
      btn.disabled = true
      btn.innerHTML = '<span style="display: inline-flex; align-items: center; gap: 0.25rem;">Carregando...</span>'
    }
    ...
    if (data.url) {
      ...
      // Se não tem URL, mostra mensagem
      alert('URL da cobrança não encontrada. Verifique se a cobrança tem boleto gerado na Conta Azul.')
    }
  } catch (error) {
    console.error('Erro ao abrir cobrança:', error)
    alert('Erro ao buscar cobrança: ' + error.message)
  } finally {
    // Restaura o botão
    const btn = event?.target?.closest('button')
    if (btn) {
